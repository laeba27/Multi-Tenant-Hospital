...
export async function PATCH(request) {
  try {
    ...
    if ((type === 'hospital' || type === 'both') && hospitalData) {
      ...
      const sanitizedHospitalData = { ...hospitalData }
      // Hospital account status is controlled by super admin approval workflow.
      delete sanitizedHospitalData.account_status

      const { error: hospitalError } = await supabase
        .from('hospitals')
        .update({
          ...sanitizedHospitalData,
          updated_at: new Date().toISOString(),
        })
