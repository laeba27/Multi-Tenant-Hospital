...
export function AiPrescribePanel({ appointmentId }) {
      ...
      setRecordings((prev) =>
        prev.map((item) =>
          item.id === recording.id
            ? { ...item, status: 'error', error: 'Audio clip is empty.' }
            : item
        )
      )
      return
  ...
  const hasTranscript = recordings.some((item) => Boolean(item.transcript))
  ...
  const handleProcessTranscript = async () => {
    ...
    if (!currentTranscript) {
      setError('Paste or type a transcript first.')
      return
    }
    ...
    try {
      ...
      setStructured(buildPrescriptionDraft({ transcript: currentTranscript }))
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap gap-3">
            <Button variant="outline" onClick={handleReextract} disabled={isProcessing}>
              Re-extract fields
            </Button>
            <Button variant="outline" onClick={handleProcessTranscript} disabled={isProcessing || !transcript.trim()}>
              Process transcript
            </Button>
          </div>
        </CardContent>
