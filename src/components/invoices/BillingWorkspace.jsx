 ...
 *
 * This used to live inside the reception page and gate on
 * `role === 'receptionist'`, which made "who may bill" a property of the job
 * title rather than of the permission. A doctor the admin had granted
 * `manage_billing` still had nowhere to do it. Now the screen asks the
 * permission, not the role, so reception and doctors mount the same component
 * and the admin's RBAC switch is what decides.
 */
export function BillingWorkspace() {
  ...
  // Any staff member has a `staff` row; the role no longer decides whether we
  // bother to load it. Patients and hospital-level admins have none, so they
  // simply resolve to no staff details rather than hanging on a spinner.
  useEffect(() => {
    if (user?.id) {
      fetchStaffDetails()
    }
  ...
  const fetchStaffDetails = async () => {
    ...
    try {
      ...
      // Not every biller has a `staff` row -- a hospital_admin holds every
      // permission but is not staff. That is a "no hospital resolved here"
      // case, not a failure, so it must not raise a red toast on a screen the
      // person is perfectly entitled to open.
      if (result.error) {
        console.warn('No staff record for billing user:', result.error)
  ...
  // Tidiness only -- createInvoice() and recordPayment() both call
  // requirePermission('manage_billing') themselves, so someone who reaches this
  // screen anyway still cannot bill.
      ...
      <DashboardLayout>
        <div className="flex items-center justify-center h-96">
          <Alert variant="destructive" className="max-w-md">
            ...
            <AlertDescription>
              You don&apos;t have permission to manage billing and payments. Please
      ...
      <DashboardLayout>
        <div className="flex items-center justify-center h-96">
          <Alert className="max-w-md">
            ...
            <AlertDescription>
              We couldn&apos;t work out which hospital to bill for from your staff
              record. Please ask your hospital administrator to check it.
            </AlertDescription>
          </Alert>
        </div>
      </DashboardLayout>
