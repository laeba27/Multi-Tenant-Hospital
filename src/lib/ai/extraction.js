const SYMPTOM_PATTERNS = [
  ...
  { key: 'dry cough', patterns: ['dry cough'] },
...
const CONTEXT_MARKERS = {
  complaints: ['complaining of', 'complaint', 'pain', 'ache', 'symptom', 'having', 'fever', 'cough'],
...
const INVESTIGATION_PATTERNS = [
  { key: 'Chest X-ray', patterns: ['chest x-ray', 'chest xray', 'x-ray chest', 'x ray chest'] },
  { key: 'X-ray', patterns: ['x-ray', 'xray', 'x ray'] },
  { key: 'CBC', patterns: ['cbc', 'complete blood count'] },
]

const ADVICE_PATTERNS = [
  { key: 'take proper rest', patterns: ['proper rest', 'take rest', 'rest'] },
  { key: 'stay hydrated', patterns: ['stay hydrated', 'hydrated', 'drink plenty of fluids', 'fluids'] },
]
...
function extractComplaints(complaintSegments, fullText) {
  ...
  segments.forEach((segment) => {
    ...
    SYMPTOM_PATTERNS.forEach(({ key, patterns }) => {
      ...
      if (!matched) return
      if (key === 'cough' && fullText.includes('dry cough')) return
...
function extractExamFindings(examSegments, fullText) {
  ...
  if (source.includes('stable') || source.includes('looks stable') || source.includes('general condition stable')) {
    findings.general_condition = findings.general_condition
      ? `${findings.general_condition}; stable`
      : 'stable'
  }
  ...
  if (source.includes('chest') && !source.includes('x-ray') && !source.includes('xray') && !source.includes('x ray')) {
...
function extractHistoryPresentIllness(complaintSegments, fullText) {
  const source = complaintSegments.length ? complaintSegments : splitSentences(fullText)
  const firstLine =
    source.find((segment) =>
      SYMPTOM_PATTERNS.some(({ patterns }) => patterns.some((pattern) => segment.includes(pattern)))
    ) || source.find(Boolean)
...
function extractInvestigations(fullText) {
  const investigations = []

  INVESTIGATION_PATTERNS.forEach(({ key, patterns }) => {
    if (patterns.some((pattern) => fullText.includes(pattern))) {
      if (key === 'X-ray' && INVESTIGATION_PATTERNS[0].patterns.some((pattern) => fullText.includes(pattern))) {
        return
      }
      investigations.push(`${key} recommended`)
    }
  })

  return dedupeBy(investigations, (item) => item)
}

function extractAdvice(fullText) {
  const advice = []

  ADVICE_PATTERNS.forEach(({ key, patterns }) => {
    if (patterns.some((pattern) => fullText.includes(pattern))) {
      advice.push(key)
    }
  })

  return dedupeBy(advice, (item) => item)
}
...
export function buildPrescriptionDraft({ transcript }) {
  ...
  const investigations = extractInvestigations(correctedText)
  const advice = extractAdvice(correctedText)
